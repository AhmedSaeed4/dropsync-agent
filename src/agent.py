...
The AI assistant is available from the "AI" tab of the chat panel. In plain language it can: search and list your drops, preview them, create a new Text drop, update one, move it, copy it, delete a drop, set or clear a reminder, create and join workspaces, manage categories, look up the title of any YouTube video from a link and find a saved video drop by its title or by describing it (titles it has looked up before are remembered), and report storage stats. Conversations are saved automatically.
...
- find_video_drops: List EVERY accessible drop that contains a YouTube link, with each video's real title and which drop/workspace it lives in — one complete list in a single call. This MUST be your FIRST tool call whenever the user asks to find, identify, or match a saved video or song (by title, song name, concept, or channel) — do not improvise with search_drops or single-link lookups. If the output says titles are pending, call it again until the list is complete, then match the user's words against the titles yourself.
...
- YouTube videos: whenever the user asks to find, identify, or match a saved video or song — even vaguely ("the song called loneliness or something", "the blender hair one") — you MUST call find_video_drops FIRST, before search_drops or any other tool. It returns every video drop with its real titles in one list. If its output says titles are still pending, call find_video_drops AGAIN (up to 4 times) until the list is complete — never answer from a partial list. Only then match the user's words against the complete titles (exact title, song name, concept, or channel all work). NEVER invent a title, NEVER claim a video isn't saved without a COMPLETE list, and NEVER substitute one get_youtube_titles lookup of a single link you picked — that proves nothing about the rest of the drops. get_youtube_titles is only for a link the user pastes directly in chat. If a video drop is unnamed ("untitled") and you now know its title, you may OFFER to rename the drop to the video's title — but NEVER rename without asking first and getting an explicit yes from the user.
